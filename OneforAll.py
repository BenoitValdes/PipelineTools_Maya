...
uiFile = os.path.join(os.path.dirname(__file__), 'UI/OneforAll.ui')				# The .ui file to load
windowTitle = 'One For All'									# The visible title of the window
windowObject = 'one_for_all'									# The name of the window object
...
class Main(form, base):
	def __init__(self, parent=None):
		...
		# Signals
		self.finalButton.clicked.connect(self.addOriginal)
		self.addButton.clicked.connect(self.addBlend)
	...
	def addOriginal(self):
		if cmds.ls( selection=True, tail=1 ):
			items = []
			for index in xrange(self.CamList.count()):
				selectedItem = self.CamList.item(index)
				items.append(selectedItem.text())
			for object in cmds.ls(selection=True):
				if not object in items:
					self.finalCamInput.setText(cmds.ls( selection=True, tail=1 )[0])
		self.checkRunButton()
	
	# check if mesh can be added in "blendshape" list
	def addBlend(self):
		if cmds.ls(selection=True):
			items = [self.finalCamInput.text()]
			for index in xrange(self.CamList.count()):
				selectedItem = self.CamList.item(index)
				items.append(selectedItem.text())
			for object in cmds.ls(selection=True):
				if not object in items:
					self.CamList.addItem(object)
		self.checkRunButton()
	...
	def delBlend(self):
		...
		listQList = []
		for i in range(self.CamList.count()):
			listQList.append( self.CamList.item(i) )			
		newList = [item for item in listQList if item.text() in listSelected]		
		for item in newList:
			self.CamList.takeItem(self.CamList.row(item))
	...
	def checkRunButton(self):
		self.runButton.setDisabled(True)
		if self.finalCamInput.text() and xrange(self.CamList.count()):
