...
uiFile = os.path.join(os.path.dirname(__file__), 'UI/treeTest.ui')				# The .ui file to load
windowTitle = 'Namespace Manager'									# The visible title of the window
windowObject = 'NamespaceManager'									# The name of the window object
...
class Main(form, base):
	def __init__(self, parent=None):
		...
		#INIT
		self.namespaceTree.setColumnCount(1)
		#
	...
	#Function		
		...
		'''
		#INIT
		self.nameSpaceList.addItems(self.listNamespace())
		#
		
	#Function		
	def listNamespace(self):
		tmpnNamespace = cmds.namespaceInfo(':', listOnlyNamespaces=True, recurse = True )
		namespace = []
		for item in tmpnNamespace:
		    if ':' in item:
		        items = item.split(':')
		        for items in items:  
		            namespace.append(items)
		    else:
		        namespace.append(item)			
		if 'UI' in namespace:
		    namespace.remove('UI')				
		if 'shared' in namespace:
